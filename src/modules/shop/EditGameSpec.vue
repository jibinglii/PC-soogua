...
<script>
	...
	import XSpecInputEdit from "./components/XSpecInputEdit";
		...
		methods: {
			async next () {
				if (!this.saving) {
					...
					await this.$http
						.post("api/v1/goods/" + this.uuid, param)
						.then(response => {
							this.saving = false;
							loading.close()
							this.$router.replace({
			...
			initSpec (spec) {
				spec.forEach((item) => {
					if (item.type == 'input') {
						this.$set(this.specValue, item.id, item.value)
					} else {
						this.$set(this.specValue, item.id, item.items.filter(item => item.checked).map(item => item.id))
					}
				})
			}
		},
